...
public class ModelHelper {
    ...
    public static CourseDTO enrich(CourseDTO course) {
        Link selfLink = linkTo(CourseController.class).slash(course.getName()).withSelfRel();
        Link enrolledLink = linkTo(CourseController.class).slash(course.getName()).slash("enrolled").withRel("enrolled");
        course.add(selfLink);
